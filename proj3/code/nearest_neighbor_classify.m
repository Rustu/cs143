...
%     size(test_image_feats)==[1500 256]
...
Useful functions:
 ...
 D = vl_alldist2(X,Y) 
    ...
    D(i,j) = sum (X(:,oints as columns vs this code (and Matlab
    i) - Y(:,j)).^2
    Note that vl_feat represents pin general) represents points as rows. So you probably want to use the
    transpose operator ' 
