...
pTrain={'M',20};
forest=forestTrain( img_features, labels, pTrain);
...
%     label_vector = (labels == i)*2 - 1;
%     [tp_rate, fp_rate, tn_rate, fn_rate] =  report_accuracy( confidences, label_vector );
    [pb] = detect_sketch_tokens(cur_img, forest, feature_params)-0.5;
...
%     beep;
%     waitforbuttonpress;
%     disp('showing single(pb/255)');
%     imshow(single(pb/255));
%     beep;
%     waitforbuttonpress;
%     pb = single(pb/255);
%     disp('showing pb');
